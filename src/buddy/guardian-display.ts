...
export type MessageType = 'idle' | 'reaction' | 'attentive' | 'council' | 'ghost' | 'debate'
...
function buildSnapshot(): GuardianDisplaySnapshot {
  ...
  // Infer message type from content markers
  let messageType: MessageType = 'idle'
  if (hasReaction) {
    if (displayText.includes('COUNCIL') || displayText.includes('>> ')) messageType = 'council'
    else if (displayText.includes('DEBATE') || displayText.includes('═══')) messageType = 'debate'
    else if (displayText.includes('...') && displayText.includes('—')) messageType = 'ghost'
    else messageType = 'reaction'
  } else if (isAttentive && attentiveTicksRemaining > 0) {
    messageType = 'attentive'
  }

  return { displayText, fading, emotionColor, emotion, messageType }
}
