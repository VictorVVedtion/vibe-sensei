...
export function CompanionSprite(): React.ReactNode {
  ...
  const { displayText, fading, emotionColor, emotion: singletonEmotion, messageType } = useGuardianDisplay();
  ...
  // Wide-form display for long messages (council, debate, ghost)
  const isWideMessage = messageType === 'council' || messageType === 'debate' || messageType === 'ghost'
  if (isWideMessage && displayText.length > 80 && columns >= 60) {
    return (
      <Box flexDirection="column" paddingX={1} flexShrink={0}>
        <Box borderStyle="double" borderColor={messageType === 'ghost' ? 'magenta' : color} paddingX={1}>
          <Text dimColor={fading}>{displayText}</Text>
        </Box>
        <Box alignSelf="flex-end">{minimalistFaceRow}</Box>
      </Box>
    )
  }

  // Static scrollback: bubble + full sprite on wide terminals
  const spriteLines = columns >= MIN_COLS_FOR_FULL_SPRITE
    ? renderSprite(companion, 0, currentEmotion)
    : null
