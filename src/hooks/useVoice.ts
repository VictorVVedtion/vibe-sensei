...
export function useVoice({
  ...
  // Tracked for the vs_voice_recording_completed analytics event.
    ...
    void finalizePromise
      .then(async finalizeSource => {
        ...
        if (
          ...
          logEvent('vs_voice_silent_drop_replay', {
            recordingDurationMs,
        ...
        logEvent('vs_voice_recording_completed', {
        ...
        if (text) {
          ...
          if (!wsConnected) {
            ...
            onErrorRef.current?.(
              'No audio detected from microphone. Check that the correct input device is selected and that Vibe Sensei has microphone access.',
            )
          } else {
  ...
  async function startRecordingSession(): Promise<void> {
    ...
    logEvent('vs_voice_recording_started', {
          ...
          onError: (error: string, opts?: { fatal?: boolean }) => {
            ...
            if (
              ...
              if (!retryUsedRef.current) {
                ...
                logEvent('vs_voice_stream_early_retry', {})
                connectionRef.current = null
                attemptGenRef.current++
                setTimeout(
