...
function logApprovalEvent(
  ...
  if (
    ...
    logEvent(
      'vs_tool_use_granted_by_classifier',
  ...
  switch (source.type) {
    case 'user':
      logEvent(
        source.permanent
          ? 'vs_tool_use_granted_in_prompt_permanent'
          : 'vs_tool_use_granted_in_prompt_temporary',
        baseMetadata(messageId, tool.name, waitMs),
      )
      break
    case 'hook':
      logEvent('vs_tool_use_granted_by_permission_hook', {
        ...baseMetadata(messageId, tool.name, waitMs),
...
function logRejectionEvent(
  ...
  if (source === 'config') {
    ...
    logEvent(
      'vs_tool_use_denied_in_config',
      baseMetadata(messageId, tool.name, undefined),
    )
    return
  }
  logEvent('vs_tool_use_rejected_in_prompt', {
    ...baseMetadata(messageId, tool.name, waitMs),
