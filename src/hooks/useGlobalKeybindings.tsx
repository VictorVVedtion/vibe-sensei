...
export function GlobalKeybindingHandlers({
  ...
  const handleToggleShowAll = useCallback(() => {
    logEvent('vs_transcript_toggle_show_all', {
  ...
  const handleToggleBrief = useCallback(() => {
    if (feature('KAIROS') || feature('KAIROS_BRIEF')) {
      ...
      logEvent('vs_brief_mode_toggled', {
        enabled: next,
        gated: false,
  ...
  const handleToggleTerminal = useCallback(() => {
    if (feature('TERMINAL_PANEL')) {
      if (!getFeatureValue_CACHED_MAY_BE_STALE('vs_terminal_panel', false)) {
        return;
      }
