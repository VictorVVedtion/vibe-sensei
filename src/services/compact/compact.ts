 ...
 * Lets the vs_compact event disambiguate same-chain loops (H2) from
    ...
    for (;;) {
      ...
      const truncated =
        ptlAttempts <= MAX_PTL_RETRIES
          ...
          : null
      if (!truncated) {
        logEvent('vs_partial_compact_failed', {
          reason:
            'prompt_too_long' as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
          ...failureMetadata,
          ptlAttempts,
...
async function streamCompactSummary({
  ...
  // 3P default: true — see comment at the other vs_compact_cache_prefix read above.
  const promptCacheSharingEnabled = getFeatureValue_CACHED_MAY_BE_STALE(
    'vs_compact_cache_prefix',
    true,
  )
  ...
  try {
      ...
      try {
        ...
        if (assistantMsg && assistantText && !assistantMsg.isApiErrorMessage) {
          ...
          if (!assistantText.startsWith(PROMPT_TOO_LONG_ERROR_MESSAGE)) {
            logEvent('vs_compact_cache_sharing_success', {
              preCompactTokenCount,
        ...
        logEvent('vs_compact_cache_sharing_fallback', {
          reason:
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (attempt < maxAttempts) {
        logEvent('vs_compact_streaming_retry', {
          attempt,
          preCompactTokenCount,
          hasStartedStreaming,
        })
  ...
  const results = await Promise.all(
    recentFiles.map(async file => {
      const attachment = await generateFileAttachment(
        ...
        'vs_post_compact_file_restore_success',
        'vs_post_compact_file_restore_error',
        'compact',
      )
