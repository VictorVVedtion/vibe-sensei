...
// Per-session bearer token for screenshot POST authentication
const CHART_TOKEN = randomUUID();
...
/**
 * Validate the Authorization: Bearer header against the per-session CHART_TOKEN.
 * Returns true if the token matches, otherwise sends a 401 and returns false.
 */
function requireScreenshotAuth(req: Request, res: Response): boolean {
  const authHeader = req.headers.authorization ?? '';
  const match = authHeader.match(/^Bearer\s+(.+)$/i);
  if (!match || match[1] !== CHART_TOKEN) {
...
function handleScreenshotPost(req: Request, res: Response): void {
  if (!requireScreenshotAuth(req, res)) return;
...
/** Get the per-session chart screenshot auth token */
export function getChartToken(): string {
  return CHART_TOKEN;
}
...
export function mountUdfRoutes(app: Express): void {
  ...
  // SECURITY: gate the chart-token handout. Stricter than the WS upgrade
  // gate — accepts only same-host origin (the chart frontend we serve) plus
  // file:/app: (Electron renderer). A no-Origin request is allowed because
  // some same-origin XHRs omit Origin; combined with the loopback bind, the
  // only such caller is a process on the same machine.
  app.get('/api/chart-token', (req: Request, res: Response) => {
    const origin = (req.headers.origin as string | undefined) ?? '';
    const host = (req.headers.host as string | undefined) ?? '';
    const allowed =
      !origin || isSameHostOrigin(origin, host) || isLocalOrigin(origin);
    if (!allowed) {
      console.warn(`[UDF] Rejected /api/chart-token from origin: ${origin}`);
      res.status(403).json({ error: 'Forbidden' });
      return;
    }
    res.json({ token: CHART_TOKEN });
  });
...
export function createUdfApp(): Express {
  ...
  // SECURITY: reflect only same-host or local-protocol origins. The server
  // binds to 127.0.0.1, so the only legitimate callers are the chart frontend
  // (same-host) and the Electron renderer (file://app://). Wildcard CORS
  // combined with the unauthenticated chart-token endpoint previously let
  // any browser tab steal the screenshot bearer token.
  app.use((req: Request, res: Response, next: () => void) => {
    const origin = req.headers.origin;
    if (origin) {
      const host = (req.headers.host as string | undefined) ?? '';
      if (isSameHostOrigin(origin, host) || isLocalOrigin(origin)) {
