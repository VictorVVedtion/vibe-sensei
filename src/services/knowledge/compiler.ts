...
// ── Path Safety (delegated to wiki-utils) ────────────────────────────────────
// writeArticleAtomic, readArticle, isSafeArticlePath imported from wiki-utils.ts
...
/** Build the master INDEX.md article with category grouping. */
function buildIndexArticle(articles: WikiArticle[], date: string): WikiArticle {
  // Also include existing articles from disk not in current compile batch
  const existingPaths = listWikiArticlesRelative()
  const allPaths = new Set<string>()
  const articleMap = new Map<string, WikiArticle>()

  for (const a of articles) {
    if (a.path === 'INDEX.md' || a.path === 'log.md') continue
    allPaths.add(a.path)
    articleMap.set(a.path, a)
  }
  for (const p of existingPaths) {
    if (p === 'INDEX.md' || p === 'log.md') continue
    allPaths.add(p)
  }
  ...
  for (const path of allPaths) {
    ...
    // Get title from compiled article or from disk
    const compiled = articleMap.get(path)
    let title: string
    let articleDate = date
    if (compiled) {
      title = extractTitle(compiled.content)
      articleDate = extractDate(compiled.content) ?? date
    } else {
      const content = readArticle(path)
      title = content ? extractTitle(content) : path.replace(/\.md$/, '')
      articleDate = content ? (extractDate(content) ?? date) : date
    }
    const wikilink = path.replace(/\.md$/, '')
    categories[dir]!.push(`- [[${wikilink}|${title}]] (${articleDate})`)
  }
...
export async function compile(opts?: { full?: boolean }): Promise<CompileResult> {
  ...
  for (const article of articles) writeArticleAtomic(article.path, article.content)
  ...
  appendLog(`compile: ${articles.length} articles updated (${mode} mode), ${newEvents.length} events processed`)
