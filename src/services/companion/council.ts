...
export class AsyncCouncil {
  ...
  async convene(context: CouncilContext): Promise<CouncilResult> {
    ...
    // Emit council debate to desktop bridge for the DebateRoom overlay
    this.emitCouncilDebate(result)
  ...
  /**
   * Emit the council debate result to the desktop bridge.
   * Maps the internal CouncilResult to the CouncilDebate IPC schema.
   * Silently swallows all errors.
   */
  private emitCouncilDebate(result: CouncilResult): void {
    try {
      if (!isDesktopMode()) return

      emitToDesktop('council_debate', {
        topic: result.triggerDetail,
        participants: result.masters.map((m) => ({
          masterName: m.name,
          position: this.archetypeToPosition(m, result),
          argument: m.stance,
          spriteId: this.nameToSpriteId(m.name),
        })),
        verdict: result.conclusion,
        recommendation: result.recommendation === 'none' ? undefined : result.recommendation,
  ...
  /**
   * Map a council master's stance to a debate position (for/against/neutral).
   * The first master in the archetype picks is conservative (against risk),
   * the second is more opportunistic (for risk), the third is neutral.
   */
  private archetypeToPosition(
    master: CouncilMasterStance,
    result: CouncilResult,
  ): 'for' | 'against' | 'neutral' {
    const idx = result.masters.indexOf(master)
    if (idx === 0) return 'against'
    if (idx === 1) return 'for'
    return 'neutral'
  }

  /**
   * Convert a master display name to a sprite file ID.
   * Matches the naming convention in assets/sprites/ (lowercase, underscored).
   */
  private nameToSpriteId(displayName: string): string {
    return displayName.toLowerCase().replace(/\s+/g, '_').replace(/[.'"]/g, '')
  }
