...
// Allows tickers like BTC, BTC/USDT, BTC-USDT, BTC/USDT/PERP, BTC/USDT:USDT.
// Rejects NUL/BEL/ANSI escape sequences and any other terminal control chars.
const SYMBOL_PATTERN = /^[A-Z0-9]+([\/\-:][A-Z0-9]+)*$/i;

function validateSymbol(symbol: unknown): asserts symbol is string {
  if (typeof symbol !== 'string' || !SYMBOL_PATTERN.test(symbol)) {
    throw new Error(
      `Invalid symbol: ${JSON.stringify(symbol)}. Must match [A-Z0-9]+(/[A-Z0-9]+)*`,
    );
  }
}
...
export class PaperExchange implements ExchangeInterface {
  ...
  constructor(config?: PaperConfig, marketDataSource?: ExchangeInterface) {
    ...
    if (!Number.isFinite(initialBalance) || initialBalance < 0) {
      throw new Error(
        `PaperExchange: initialBalance must be a finite non-negative number, got: ${initialBalance}`,
      );
    }
    const feeRate = config?.feeRate ?? DEFAULT_FEE_RATE;
    if (!Number.isFinite(feeRate) || feeRate < 0 || feeRate >= 1) {
      throw new Error(
        `PaperExchange: feeRate must be a finite number in [0, 1), got: ${feeRate}`,
      );
    }
  ...
  updatePrice(symbol: string, price: number): void {
    ...
    if (!Number.isFinite(price) || price <= 0) {
      throw new Error(`Price must be a finite positive number, got: ${price}`);
    }
  ...
  /**
   * Lazily connect the market data source. Returns true if available.
   */
  ...
  private validateOrderRequest(req: OrderRequest): void {
    ...
    if (!VALID_ORDER_TYPES.has(req.type as string)) {
      throw new Error(
        `Invalid order type: ${req.type}. Must be one of: market, limit, stop_loss`,
      );
    }
    if (!Number.isFinite(req.quantity) || req.quantity <= 0) {
      throw new Error(
        `Quantity must be a finite positive number, got: ${req.quantity}`,
      );
    }
    if (
      req.type === 'limit' &&
      (req.price == null || !Number.isFinite(req.price) || req.price <= 0)
    ) {
      throw new Error('Limit orders require a finite positive price');
    }
    if (
      req.type === 'stop_loss' &&
      (req.stopPrice == null ||
      ...
      throw new Error('Stop loss orders require a finite positive stopPrice');
    }
  }
