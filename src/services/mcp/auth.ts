...
/**
 * Failure reasons for the `vs_mcp_oauth_refresh_failure` event. Values
...
/**
 * Failure reasons for the `vs_mcp_oauth_flow_error` event. Values are
 ...
 *
 * Per RFC 7009, public clients (like Vibe Sensei) should authenticate by including
...
async function performMCPXaaAuth(
  ...
  try {
    ...
    logEvent('vs_mcp_oauth_flow_success', {
      authMethod:
    ...
    logEvent('vs_mcp_oauth_flow_failure', {
      authMethod:
        'xaa' as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
      xaaFailureStage:
...
export async function performMCPOAuthFlow(
  ...
  if (serverConfig.oauth?.xaa) {
    ...
    logEvent('vs_mcp_oauth_flow_start', {
      isOAuthFlow: true,
      authMethod:
        'xaa' as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
      transportType:
  ...
  try {
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to Vibe Sensei.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
    ...
    logEvent('vs_mcp_oauth_flow_error', {
      flowAttemptId:
        flowAttemptId as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
      reason:
 ...
 * to the PKCE flow. See github.com/VictorVVedtion/vibe-sensei/issues/28258.
 */
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `Vibe Sensei (${this.serverName})`,
  ...
  private async _doRefresh(
    ...
    const emitRefreshEvent = (
      ...
      logEvent(
        outcome === 'success'
          ? 'vs_mcp_oauth_refresh_success'
          : 'vs_mcp_oauth_refresh_failure',
        {
