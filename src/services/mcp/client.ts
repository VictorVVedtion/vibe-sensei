...
// (@vibe-sensei/computer-use-input + @vibe-sensei/computer-use-swift). Runtime-gated by
// GrowthBook vs_malort_pedway (see gates.ts).
 ...
 * emits vs_mcp_server_needs_auth, caches the needs-auth entry, and returns
...
function handleRemoteAuthFailure(
  ...
  logEvent('vs_mcp_server_needs_auth', {
    transportType:
...
export function createClaudeAiProxyFetch(innerFetch: FetchLike): FetchLike {
  return async (url, init) => {
    ...
    logEvent('vs_mcp_claudeai_proxy_401', {
      tokenChanged:
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
        ...
        const { createClaudeForChromeMcpServer } = await import(
          '@vibe-sensei/claude-for-chrome-mcp'
        )
      ...
      try {
        ...
        if (serverRef.type === 'sse' && error instanceof Error) {
          ...
          logEvent('vs_mcp_ide_server_connection_failed', {
      ...
      if (serverRef.type === 'sse-ide' || serverRef.type === 'ws-ide') {
        ...
        logEvent('vs_mcp_ide_server_connection_succeeded', {
      ...
      logEvent('vs_mcp_server_connection_succeeded', {
      ...
      const connectionDurationMs = Date.now() - connectStartTime
      logEvent('vs_mcp_server_connection_failed', {
        connectionDurationMs,
  ...
  return new Promise(resolve => {
    ...
    getMcpToolsCommandsAndResources(result => {
      ...
      if (completedCount >= pendingCount) {
        ...
        logEvent('vs_mcp_tools_commands_loaded', {
...
export async function processMCPResult(
  ...
  if (isEnvDefinedFalsy(process.env.ENABLE_MCP_LARGE_OUTPUT_FILES)) {
    logEvent('vs_mcp_large_result_handled', {
      outcome: 'truncated',
      reason: 'env_disabled',
      sizeEstimateTokens,
...
async function callMCPTool({
  ...
  try {
    ...
    if (codeIndexingTool) {
      logEvent('vs_code_indexing_tool_used', {
    ...
    if (e instanceof Error) {
      ...
      if (errorCode === 401 || e instanceof UnauthorizedError) {
        ...
        logEvent('vs_mcp_tool_call_auth_error', {})
      ...
      if (isSessionExpired || isConnectionClosedOnHttp) {
        ...
        logEvent('vs_mcp_session_expired', {})
        await clearServerCache(name, config)
