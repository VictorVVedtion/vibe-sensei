...
export function setupVscodeSdkMcp(sdkClients: MCPServerConnection[]): void {
  ...
  if (client && client.type === 'connected') {
    ...
    const gates: Record<string, boolean | string> = {
      vs_vscode_review_upsell: checkStatsigFeatureGate_CACHED_MAY_BE_STALE(
        'vs_vscode_review_upsell',
      ),
      vs_vscode_onboarding: checkStatsigFeatureGate_CACHED_MAY_BE_STALE(
        'vs_vscode_onboarding',
      ),
      // Browser support.
      vs_quiet_fern: getFeatureValue_CACHED_MAY_BE_STALE(
        'vs_quiet_fern',
        false,
      ),
      // In-band OAuth via claude_authenticate (vs. extension-native PKCE).
      vs_vscode_cc_auth: getFeatureValue_CACHED_MAY_BE_STALE(
        'vs_vscode_cc_auth',
        false,
      ),
    }
    ...
    if (autoModeState !== undefined) {
      gates.vs_auto_mode_state = autoModeState
    }
