...
// OAuth credentials as Vibe Sensei.  The endpoint uses conversation_engine
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as Vibe Sensei — available when the
...
export async function connectVoiceStream(
  ...
  // non-browser clients (VictorVVedtion/vibe-sensei#34094). Same private-api
  ...
  if (isNova3) {
    ...
    logForDebugging('[voice_stream] Nova 3 gate enabled (vs_cobalt_frost)')
  }
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (VictorVVedtion/vibe-sensei#40510).
