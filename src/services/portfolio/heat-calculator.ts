...
import { computeAssetValueUSD } from './valuation.js'
 ...
 * Without stop-loss: risk = computeAssetValueUSD(position) * 10%
 */
function calculatePositionRisk(
  ...
  if (stopOrder && stopOrder.stopPrice != null) {
    ...
    hasStopLoss = false
    const notional = computeAssetValueUSD(
      position,
      'spot',
      { price: position.currentPrice },
    )
    risk = notional * DEFAULT_ADVERSE_MOVE
  }
