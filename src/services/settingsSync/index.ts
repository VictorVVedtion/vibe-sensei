 ...
 *
 * Syncs user settings and memory files across Vibe Sensei environments.
 *
...
export async function uploadUserSettingsInBackground(): Promise<void> {
  try {
    if (
      ...
      logEvent('vs_settings_sync_upload_skipped_ineligible', {})
      return
    }
    ...
    if (!result.success) {
      ...
      logEvent('vs_settings_sync_upload_fetch_failed', {})
      return
    }
    ...
    if (entryCount === 0) {
      ...
      logEvent('vs_settings_sync_upload_skipped', {})
      return
    }
    ...
    if (uploadResult.success) {
      ...
      logEvent('vs_settings_sync_upload_success', { entryCount })
    } else {
      logForDiagnosticsNoPII('warn', 'settings_sync_upload_failed')
      logEvent('vs_settings_sync_upload_failed', { entryCount })
    }
  } catch {
...
async function doDownloadUserSettings(
  ...
  if (feature('DOWNLOAD_USER_SETTINGS')) {
    try {
      if (
        !getFeatureValue_CACHED_MAY_BE_STALE('vs_strap_foyer', false) ||
        !isUsingOAuth()
      ) {
        logForDiagnosticsNoPII('info', 'settings_sync_download_skipped')
        logEvent('vs_settings_sync_download_skipped', {})
        return false
      }
      ...
      if (!result.success) {
        ...
        logEvent('vs_settings_sync_download_fetch_failed', {})
        return false
      }

      if (result.isEmpty) {
        logForDiagnosticsNoPII('info', 'settings_sync_download_empty')
        logEvent('vs_settings_sync_download_empty', {})
        return false
      }
