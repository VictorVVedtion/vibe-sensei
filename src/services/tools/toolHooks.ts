    ...
    for await (const result of executePostToolHooks(
      ...
      try {
        ...
        if (
          ...
          logEvent('vs_post_tool_hooks_cancelled', {
        ...
        logEvent('vs_post_tool_hook_error', {
          messageID:
    ...
    for await (const result of executePostToolUseFailureHooks(
      ...
      try {
        ...
        if (
          ...
          logEvent('vs_post_tool_failure_hooks_cancelled', {
            toolName: sanitizeToolNameForAnalytics(tool.name),
            queryChainId: toolUseContext.queryTracking
              ?.chainId as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
            queryDepth: toolUseContext.queryTracking?.depth,
        ...
        const postToolDurationMs = Date.now() - postToolStartTime
        logEvent('vs_post_tool_failure_hook_error', {
          messageID:
            messageId as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
          toolName: sanitizeToolNameForAnalytics(tool.name),
          isMcp: tool.isMcp ?? false,
    ...
    for await (const result of executePreToolHooks(
      ...
      try {
        ...
        if (toolUseContext.abortController.signal.aborted) {
          logEvent('vs_pre_tool_hooks_cancelled', {
            toolName: sanitizeToolNameForAnalytics(tool.name),
        ...
        logEvent('vs_pre_tool_hook_error', {
          messageID:
            messageId as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
          toolName: sanitizeToolNameForAnalytics(tool.name),
          isMcp: tool.isMcp ?? false,
