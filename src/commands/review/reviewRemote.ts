...
export async function checkOverageGate(): Promise<OverageGate> {
  ...
  if (!extraUsage?.is_enabled) {
    logEvent('vs_review_overage_not_enabled', {})
  ...
  if (available < 10) {
    logEvent('vs_review_overage_low_balance', { available })
    return { kind: 'low-balance', available }
  }

  if (!sessionOverageConfirmed) {
    logEvent('vs_review_overage_dialog_shown', {})
...
export async function launchRemoteReview(
  ...
  if (!eligibility.eligible) {
    ...
    if (blockers.length > 0) {
      logEvent('vs_review_remote_precondition_failed', {
  ...
  const raw = getFeatureValue_CACHED_MAY_BE_STALE<Record<
    string,
    unknown
  > | null>('vs_review_bughunter_config', null)
  ...
  if (isPrNumber) {
    ...
    if (!session) {
      logEvent('vs_review_remote_teleport_failed', {})
      return [
        {
          type: 'text',
  ...
  logEvent('vs_review_remote_launched', {})
