...
function getUltraplanModel(): string {
  return getFeatureValue_CACHED_MAY_BE_STALE('vs_ultraplan_model', ALL_MODEL_CONFIGS.opus46.firstParty);
}
  ...
  void (async () => {
    try {
      const {
        ...
        if (phase === 'needs_input') logEvent('vs_ultraplan_awaiting_input', {});
      ...
      logEvent('vs_ultraplan_approved', {
      ...
      if (executionTarget === 'remote') {
        ...
        enqueuePendingNotification({
          value: [`Ultraplan approved — executing in Vibe Sensei on the web. Follow along at: ${url}`, '', 'Results will land as a pull request when the remote session finishes. There is nothing to do here.'].join('\n'),
...
export async function launchUltraplan(opts: {
  ...
  if (active || ultraplanLaunching) {
    logEvent('vs_ultraplan_create_failed', {
  ...
  if (!blurb && !seedPlan) {
    ...
    'Usage: /ultraplan \\<prompt\\>, or include "ultraplan" anywhere', 'in your prompt', '', 'Advanced multi-agent plan mode with our most powerful model', '(Opus). Runs in Vibe Sensei on the web. When the plan is ready,', 'you can execute it in the web session or send it back here.', 'Terminal stays free while the remote plans.', 'Requires /login.', '', `Terms: ${CCR_TERMS_URL}`].join('\n');
  }
...
async function launchDetached(opts: {
  ...
  try {
    ...
    logEvent('vs_ultraplan_launched', {
...
export default {
  type: 'local-jsx',
  name: 'ultraplan',
  description: `~10–30 min · Vibe Sensei on the web drafts an advanced plan you can edit and approve. See ${CCR_TERMS_URL}`,
  argumentHint: '<prompt>',
