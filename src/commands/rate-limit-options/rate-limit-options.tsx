...
function RateLimitOptionsMenu(t0) {
  ...
  const buyFirst = getFeatureValue_CACHED_MAY_BE_STALE("vs_jade_anvil_4", false);
  let t3;
  bb0: {
    let actionOptions;
  ...
  if ($[13] !== onDone) {
    t4 = function handleCancel() {
      logEvent("vs_rate_limit_options_menu_cancel", {});
      onDone(undefined, {
        display: "skip"
      });
    };
  ...
  if ($[15] !== context || $[16] !== handleCancel || $[17] !== onDone) {
    t5 = function handleSelect(value) {
      if (value === "upgrade") {
        logEvent("vs_rate_limit_options_menu_select_upgrade", {});
        ...
        if (value === "extra-usage") {
          logEvent("vs_rate_limit_options_menu_select_extra_usage", {});
