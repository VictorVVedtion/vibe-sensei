...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  // Skip permission checks in internal_env
  ...
  if (await isQualifiedForGrove()) {
    ...
    if (decision === 'escape') {
      logEvent('vs_grove_policy_exited', {});
  ...
  if (feature('KAIROS') || feature('KAIROS_CHANNELS')) {
    // gateChannelServer and ChannelsNotice read vs_harbor after this
    ...
    if (getAllowedChannels().length > 0 || (devChannels?.length ?? 0) > 0) {
      await checkGate_CACHED_OR_BLOCKING('vs_harbor');
    }
    if (devChannels && devChannels.length > 0) {
      ...
      // Skip the dialog when channels are blocked (vs_harbor off or no
