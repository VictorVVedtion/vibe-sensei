...
export type WebSocketTransportOptions = {
  ...
  /** Gates the vs_ws_transport_* telemetry events. Set true at the
...
export class WebSocketTransport implements Transport {
  ...
  private handleOpenEvent(): void {
    ...
    if (this.isBridge && this.reconnectStartTime !== null) {
      logEvent('vs_ws_transport_reconnected', {
  ...
  private handleConnectionError(closeCode?: number): void {
    ...
    if (this.isBridge) {
      ...
      logEvent('vs_ws_transport_closed', {
        closeCode,
        msSinceLastActivity:
    ...
    if (elapsed < DEFAULT_RECONNECT_GIVE_UP_MS) {
      ...
      if (this.isBridge) {
        logEvent('vs_ws_transport_reconnecting', {
