  ...
  if (loading) {
    return <Box flexDirection="column" padding={1}>
        <Box flexDirection="row">
          <Spinner />
          <Text bold>Loading Vibe Sensei sessions…</Text>
        </Box>
        <Text dimColor>
          {retrying ? 'Retrying…' : 'Fetching your Vibe Sensei sessions…'}
        </Text>
      </Box>;
  }
  if (loadErrorType) {
    return <Box flexDirection="column" padding={1}>
        <Text bold color="error">
          Error loading Vibe Sensei sessions
        </Text>
  ...
  switch (errorType) {
    ...
    case 'other':
      return <Box marginY={1} flexDirection="row">
          <Text dimColor>Sorry, Vibe Sensei encountered an error</Text>
        </Box>;
  }
}
