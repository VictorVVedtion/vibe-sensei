...
export function PermissionPrompt(t0) {
    ...
    t4 = value_0 => {
      ...
      if (type_0 === "accept") {
        if (acceptInputMode) {
          ...
          logEvent("vs_accept_feedback_mode_collapsed", analyticsProps);
        } else {
          setAcceptInputMode(true);
          setAcceptFeedbackModeEntered(true);
          logEvent("vs_accept_feedback_mode_entered", analyticsProps);
        }
      } else {
        if (type_0 === "reject") {
          if (rejectInputMode) {
            setRejectInputMode(false);
            logEvent("vs_reject_feedback_mode_collapsed", analyticsProps);
          } else {
            setRejectInputMode(true);
            setRejectFeedbackModeEntered(true);
            logEvent("vs_reject_feedback_mode_entered", analyticsProps);
          }
        }
      }
    };
    ...
    t5 = value_1 => {
      ...
      if (option_0.feedbackConfig) {
        ...
        if (option_0.feedbackConfig.type === "accept") {
          logEvent("vs_accept_submitted", analyticsProps_0);
        } else {
          if (option_0.feedbackConfig.type === "reject") {
            logEvent("vs_reject_submitted", analyticsProps_0);
          }
        }
      }
  ...
  if ($[31] !== onCancel || $[32] !== setAppState) {
    t7 = () => {
      logEvent("vs_permission_request_escape", {});
      setAppState(_temp);
      onCancel?.();
    };
    $[31] = onCancel;
