...
export function BypassPermissionsModeDialog(t0) {
  ...
  if ($[1] !== onAccept) {
    t2 = function onChange(value) {
      bb3: switch (value) {
        case "accept":
          {
            logEvent("vs_bypass_permissions_mode_dialog_accept", {});
  ...
  let t5;
  if ($[5] !== onChange) {
    t5 = <Dialog title="WARNING: Vibe Sensei running in Bypass Permissions mode" color="error" onCancel={handleEscape}>{t3}<Select options={t4} onChange={value_0 => onChange(value_0 as 'accept' | 'decline')} /></Dialog>;
    $[5] = onChange;
    $[6] = t5;
  } else {
    t5 = $[6];
...
function _temp() {
  logEvent("vs_bypass_permissions_mode_dialog_shown", {});
}
