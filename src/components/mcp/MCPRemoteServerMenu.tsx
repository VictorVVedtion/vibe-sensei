...
export function MCPRemoteServerMenu({
  ...
  const handleClaudeAIAuthComplete = React.useCallback(async () => {
    ...
    try {
      ...
      logEvent('vs_claudeai_mcp_auth_completed', {
        success
      });
      if (success) {
        onComplete?.(`Authentication successful. Connected to ${server.name}.`);
      } else if (result.client.type === 'needs-auth') {
        onComplete?.('Authentication successful, but server still requires authentication. You may need to manually restart Vibe Sensei.');
      } else {
        onComplete?.('Authentication successful, but server reconnection failed. You may need to manually restart Vibe Sensei for the changes to take effect.');
      }
    } catch (err) {
      logEvent('vs_claudeai_mcp_auth_completed', {
        success: false
  ...
  const handleClaudeAIClearAuthComplete = React.useCallback(async () => {
    ...
    logEvent('vs_claudeai_mcp_clear_auth_completed', {});
  ...
  const handleToggleEnabled = React.useCallback(async () => {
    ...
    try {
      ...
      if (server.config.type === 'claudeai-proxy') {
        logEvent('vs_claudeai_mcp_toggle', {
  ...
  const handleAuthenticate = React.useCallback(async () => {
    ...
    try {
      ...
      if (server.config) {
        ...
        logEvent('vs_mcp_auth_config_authenticate', {
  ...
  const handleClearAuth = async () => {
    ...
    if (server.config) {
      ...
      await revokeServerTokens(server.name, server.config);
      logEvent('vs_mcp_auth_config_clear', {});
          ...
          switch (value_0) {
            ...
            case 'reconnectMcpServer':
              ...
              try {
                ...
                if (server.config.type === 'claudeai-proxy') {
                  logEvent('vs_claudeai_mcp_reconnect', {
