...
import { getCompactFace } from '../../services/terminal-image.js';

// Lazy-cached compact face from assigned guardian
let _guardianFace: string | null | undefined;
function getGuardianFace(): string | null {
  if (_guardianFace !== undefined) return _guardianFace;
  try {
    const { getCompanion } = require('../../buddy/companion.js') as typeof import('../../buddy/companion.js');
    const companion = getCompanion();
    if (companion) {
      _guardianFace = getCompactFace(companion.species);
    } else {
      _guardianFace = null;
    }
  } catch {
    ...
    default:
      {
        ...
        const guardianFace = getGuardianFace();
        let t5;
        if ($[25] !== text) {
          t5 = <Box flexDirection="column">{guardianFace && <Text dimColor>{guardianFace} </Text>}<Markdown>{text}</Markdown></Box>;
          $[25] = text;
          $[26] = t5;
        } else {
          t5 = $[26];
        ...
        if ($[30] !== t2 || $[31] !== t3 || $[32] !== t6) {
          t7 = <Box alignItems="flex-start" flexDirection="column" marginTop={t2} width="100%" backgroundColor={t3} borderStyle="single" borderLeft={true} borderRight={false} borderTop={false} borderBottom={false} borderColor="rgb(156,255,147)" paddingLeft={1}>{t5}</Box>;
          $[30] = t2;
          $[31] = t3;
          $[32] = t6;
          $[33] = t7;
