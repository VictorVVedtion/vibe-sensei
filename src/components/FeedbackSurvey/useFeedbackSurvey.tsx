...
export function useFeedbackSurvey(messages: Message[], isLoading: boolean, submitCount: number, surveyType: FeedbackSurveyType = 'session', hasActivePrompt: boolean = false): {
  ...
  const config = useDynamicConfig<FeedbackSurveyConfig>('vs_feedback_survey_config', DEFAULT_FEEDBACK_SURVEY_CONFIG);
  const badTranscriptAskConfig = useDynamicConfig<TranscriptAskConfig>('vs_bad_survey_transcript_ask_config', DEFAULT_TRANSCRIPT_ASK_CONFIG);
  const goodTranscriptAskConfig = useDynamicConfig<TranscriptAskConfig>('vs_good_survey_transcript_ask_config', DEFAULT_TRANSCRIPT_ASK_CONFIG);
  ...
  const onOpen = useCallback((appearanceId: string) => {
    ...
    logEvent('vs_feedback_survey_event', {
