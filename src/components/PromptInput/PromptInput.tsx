...
function PromptInput({
  ...
  useEffect(() => {
    if (feature('ULTRAPLAN') && ultraplanTriggers.length) {
      addNotification({
        ...
        text: 'This prompt will launch an ultraplan session in Vibe Sensei on the web',
        priority: 'immediate',
        timeoutMs: 5000
      });
    } else {
  ...
  const onSubmit = useCallback(async (inputParam: string, isSubmittingSlashCommand = false) => {
    ...
    if (activeAgent.type !== 'leader' && onAgentSubmit) {
      logEvent('vs_transcript_input_to_teammate', {});
  ...
  const handleExternalEditor = useCallback(async () => {
    logEvent('vs_external_editor_used', {});
    ...
    if (feature('TRANSCRIPT_CLASSIFIER')) {
      if (showAutoModeOptIn || autoModeOptInTimeoutRef.current) {
        if (showAutoModeOptIn) {
          logEvent('vs_auto_mode_opt_in_dialog_decline', {});
        }
  ...
  const handleModelSelect = useCallback((model: string | null, _effort: EffortLevel | undefined) => {
    ...
    logEvent('vs_model_picker_hotkey', {
  ...
  const handleThinkingSelect = useCallback((enabled: boolean) => {
    ...
    logEvent('vs_thinking_toggled_hotkey', {
      enabled
    });
    addNotification({
  ...
  return <Box flexDirection="column" marginTop={briefOwnsGap ? 0 : 1}>
      ...
      {swarmBanner ? <>
        ...
        </> : <Box flexDirection="row" alignItems="flex-start" justifyContent="flex-start" borderColor={getBorderColor()} borderStyle="single" borderLeft={false} borderRight={false} borderBottom width="100%" borderText={buildBorderText(showFastIcon ?? false, showFastIconHint, fastModeCooldown)}>
          <PromptInputModeIndicator mode={mode} isLoading={isLoading} viewingAgentName={viewingAgentName} viewingAgentColor={viewingAgentColor} />
          <Box flexGrow={1} flexShrink={1} onClick={handleInputClick}>
            {textInputElement}
          </Box>
...
function buildBorderText(showFastIcon: boolean, showFastIconHint: boolean, fastModeCooldown: boolean): BorderTextOptions | undefined {
  const guardLabel = chalk.dim('[') + chalk.rgb(156, 255, 147)('GUARD_SHELL') + chalk.dim(']');
  if (!showFastIcon) {
    return {
      content: ` ${guardLabel} `,
      position: 'top',
  ...
  return {
    content: ` ${guardLabel}${chalk.dim(' ─ ')}${fastSeg} `,
    position: 'top',
    align: 'start',
    offset: 0
  };
