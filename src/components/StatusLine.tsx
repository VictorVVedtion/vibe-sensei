...
function buildStatusLineCommandInput(permissionMode: PermissionMode, exceeds200kTokens: boolean, settings: ReadonlySettings, messages: Message[], addedDirs: string[], mainLoopModel: ModelName, vimMode?: VimMode): StatusLineCommandInput {
  ...
  return {
    ...
    // Guardian info for Vibe Sensei trading mode
    ...(() => {
      try {
        const { getCompanion } = require('../buddy/companion.js') as typeof import('../buddy/companion.js')
        const { MASTER_NAMES, RARITY_STARS } = require('../buddy/types.js') as typeof import('../buddy/types.js')
        const companion = getCompanion()
        if (companion) {
          const name = MASTER_NAMES[companion.species as keyof typeof MASTER_NAMES] ?? String(companion.species)
          const stars = RARITY_STARS[companion.rarity] ?? '★'
          return { guardian: { name, rarity: companion.rarity, stars } }
        }
      } catch {}
      return {}
    })()
...
function StatusLineInner({
  ...
  // would leak into this session's statusline (VictorVVedtion/vibe-sensei#37596).
  ...
  useEffect(() => {
    ...
    if (statusLine) {
      ...
      // and vs_status_line_mount above fires anyway so telemetry looks fine.
