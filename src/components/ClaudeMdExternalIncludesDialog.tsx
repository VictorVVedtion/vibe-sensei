...
export function ClaudeMdExternalIncludesDialog(t0) {
  ...
  if ($[1] !== onDone) {
    t2 = value => {
      if (value === "no") {
        logEvent("vs_claude_md_external_includes_dialog_declined", {});
        saveCurrentProjectConfig(_temp2);
      } else {
        logEvent("vs_claude_md_external_includes_dialog_accepted", {});
  ...
  if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
    t8 = <Text dimColor={true}>Important: Only use Vibe Sensei with files you trust. Accessing untrusted files may pose security risks{" "}<Link url="https://code.claude.com/docs/en/security" />{" "}</Text>;
    $[8] = t8;
  } else {
    t8 = $[8];
  }
...
function _temp() {
  logEvent("vs_claude_md_includes_dialog_shown", {});
}
