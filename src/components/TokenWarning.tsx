...
export function TokenWarning(t0) {
  ...
  if (feature("REACTIVE_COMPACT")) {
    if (getFeatureValue_CACHED_MAY_BE_STALE("vs_cobalt_raccoon", false)) {
  ...
  const usedPercent = 100 - displayPercentLeft;
  ...
  if ($[9] !== autocompactLabel || $[10] !== isAboveErrorThreshold || $[11] !== percentLeft) {
    t4 = <Box flexDirection="row" gap={1}><Text color="rgb(156,255,147)" bold>[CONTEXT]</Text><StatsBar value={usedPercent} max={100} warnAt={80} errorAt={90} /><Text dimColor wrap="truncate">{showAutoCompactWarning ? (upgradeMessage ? `${autocompactLabel} \u00b7 ${upgradeMessage}` : autocompactLabel) : (upgradeMessage ? `${percentLeft}% remaining \u00b7 ${upgradeMessage}` : `${percentLeft}% remaining \u00b7 /compact`)}</Text></Box>;
    $[9] = autocompactLabel;
