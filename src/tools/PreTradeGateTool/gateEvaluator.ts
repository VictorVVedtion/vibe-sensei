 ...
 *   2b. Vertical-specific checks (futures leverage, liquidation, funding, margin)
...
import { computePortfolioValueUSD } from '../../services/portfolio/valuation.js'
...
import { getThresholds, type ThresholdConfig } from '../../buddy/thresholds.js'
...
import { getVerticalChecks } from '../../buddy/checks/vertical-dispatcher.js'
...
function aggregateStatus(checks: CheckResult[]): GateStatus {
  if (checks.some(c => c.status === 'emergency')) return 'emergency'
...
function buildSummary(status: GateStatus, checks: CheckResult[]): string {
  const emergencyCount = checks.filter(c => c.status === 'emergency').length
  ...
  switch (status) {
    ...
    case 'emergency':
      return `EMERGENCY: ${emergencyCount} critical issue${emergencyCount > 1 ? 's' : ''} — trading must stop`
  }
}

/** Collect actionable recommendations from non-passing checks. */
...
export async function evaluateGate(input: GateInput): Promise<RiskGateResult> {
  ...
  // Use valuation-based portfolio value when vertical context is present
  // (cross-venue USD normalization); otherwise fall back to raw balance sum
  const equity = input.verticalContext
  ...
  // Compute dynamic thresholds from guardian archetype (optional)
  let dynThresholds: Record<string, ThresholdConfig> | null = null
  try {
    const companion = getCompanion()
    if (companion) {
      const master = companion.species as import('../../buddy/types.js').Master
      const archetype = getMasterArchetype(master)
      // Map gate regime type to threshold regime type
      const thresholdRegime = regime === 'trending_up' || regime === 'trending_down'
        ? 'trending' as const
        : regime === 'ranging'
          ? 'ranging' as const
          : null
      dynThresholds = getThresholds(archetype, companion.stats, thresholdRegime)
    }
  } catch {
    // Dynamic thresholds are optional — fall back to hardcoded defaults
  }

  const standardChecks: CheckResult[] = [
    checkPortfolioHeat(input, positions, balances, dynThresholds?.position_size),
    checkSinglePositionRisk(input, balances, entryPrice),
    checkConcentrationGate(input, positions, balances, entryPrice, dynThresholds?.concentration),
  ...
  // ── Phase 2b: Vertical-specific checks ──────────────────────────────────
  const verticalChecks = getVerticalChecks(input.verticalContext, ticker.last)
  ...
  const allChecks = [...circuitChecks, ...standardChecks, ...verticalChecks, atrCheck]
...
export function formatGateResult(
  ...
  // Separate circuit breaker, standard checks, vertical checks, and ATR advisor
  const circuitChecks = result.checks.filter(c => c.name.startsWith('Circuit:'))
  const verticalChecks = result.checks.filter(c => c.name.startsWith('Futures:'))
  const atrChecks = result.checks.filter(c => c.name === 'ATR Stop')
  const standardChecks = result.checks.filter(
    c => !c.name.startsWith('Circuit:') && c.name !== 'ATR Stop' && !c.name.startsWith('Futures:'),
  )
  ...
  // Vertical-specific checks section
  if (verticalChecks.length > 0) {
    lines.push('')
    lines.push('--- Futures Risk ----------------------------------------')
