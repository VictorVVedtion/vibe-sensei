...
export type GlobalConfig = {
  ...
  firstStartTime?: string // ISO timestamp when Vibe Sensei was first started on this machine
  ...
  // Vibe Sensei usage tracking
  claudeCodeFirstTokenDate?: string // ISO timestamp of the user's first Vibe Sensei OAuth token
  ...
  // Vibe Sensei hint protocol state (<claude-code-hint /> tags from CLIs/SDKs).
  ...
  // Used with vs_cicada_nap_ms to throttle API calls
...
export function saveGlobalConfig(
  ...
  try {
    ...
    if (wouldLoseAuthState(currentConfig)) {
      ...
      logEvent('vs_config_auth_loss_prevented', {})
      return
    }
...
function reportConfigCacheStats(): void {
  ...
  if (total > 0) {
    logEvent('vs_config_cache_stats', {
...
function saveConfigWithLock<A extends object>(
  ...
  try {
    ...
    if (lockTime > 100) {
      ...
      logEvent('vs_config_lock_contention', {
    ...
    if (lastReadFileStats && file === getGlobalClaudeFile()) {
      try {
        ...
        if (
          ...
          logEvent('vs_config_stale_write', {
    ...
    if (error instanceof ConfigParseError) {
      ...
      if (!insideGetConfig) {
        ...
        try {
          ...
          logEvent('vs_config_parse_error', {
