/**
 * Marketplace manager for Vibe Sensei plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Vibe Sensei bug, shouldn't hit the error file.
    return {}
  }
}
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Vibe Sensei version. Run 'claude marketplace remove ${name}' and ` +
    ...
    if (name === OFFICIAL_MARKETPLACE_NAME) {
      ...
      if (
        !getFeatureValue_CACHED_MAY_BE_STALE(
          'vs_plugin_official_mkt_git_fallback',
          true,
        )
      ) {
        logForDebugging(
