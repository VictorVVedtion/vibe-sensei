...
/**
 * Vibe Sensei trading commands — surfaced in their own category at the top
 * of the picker so users can find them without scrolling past 80 inherited
 * Vibe Sensei commands. Identification is by name because the underlying
 * Command type doesn't carry a vendor tag (would require Command type widening
 * + edits to every command file). Adding a new sensei command? Add its name
 * here so it gets the priority placement and 'sensei' tag in the picker.
 */
const SENSEI_COMMAND_NAMES = new Set<string>([
  'positions',
  'balance',
  'buy',
  'sell',
...
/** Returns true when a command should be grouped under the Vibe Sensei picker section. */
function isSenseiCommand(cmd: Command): boolean {
  return SENSEI_COMMAND_NAMES.has(getCommandName(cmd))
}
...
function createCommandSuggestionItem(
  ...
  const sensei = isSenseiCommand(cmd)
  ...
  return {
    ...
    tag: sensei ? 'sensei' : isWorkflow ? 'workflow' : undefined,
...
export function generateCommandSuggestions(
  ...
  if (query === '') {
    ...
    // Sensei commands are excluded — they always render in their dedicated
    // block at the very top, regardless of usage. This keeps the sensei
    // block at exactly 12 entries always; without this exclusion the block
    // would shrink as the user uses commands (because the dedup loop below
    // would pull them out and recentlyUsed would steal them).
    const recentlyUsed: Command[] = []
    const commandsWithScores = visibleCommands
      .filter(cmd => cmd.type === 'prompt' && !isSenseiCommand(cmd))
      .map(cmd => ({
        cmd,
    ...
    // senseiCommands renders FIRST so the trading suite is visible without
    // scrolling past 80+ inherited Vibe Sensei commands.
    const senseiCommands: Command[] = []
    ...
    visibleCommands.forEach(cmd => {
      ...
      if (isSenseiCommand(cmd)) {
        senseiCommands.push(cmd)
      } else if (cmd.type === 'local' || cmd.type === 'local-jsx') {
    ...
    senseiCommands.sort(sortAlphabetically)
    ...
    // Order: Vibe Sensei trading suite → recently used → built-in → user
    // → project → policy → other. The sensei block goes ABSOLUTE FIRST so
    // the user never has to scroll past inherited Vibe Sensei skills to
    // find their trading commands. This is the headline feature of the
    // fork — discoverability matters more than recency-based ranking.
    return [
      ...senseiCommands,
      ...recentlyUsed,
      ...builtinCommands,
  ...
  // Stable partition: sensei matches first, then everything else. Each
  // side preserves the Fuse ordering above (exact/prefix priority + Fuse
  // score + usage tiebreaker). Sensei always wins ties at the partition
  // boundary so the Vibe Sensei trading suite stays discoverable even
  // when the query also matches inherited Vibe Sensei commands (e.g.
  // /b → /balance and /buy land before /branch and /btw).
  const senseiFirst: typeof sortedResults = []
  const nonSensei: typeof sortedResults = []
  for (const result of sortedResults) {
    if (isSenseiCommand(result.r.item.command)) {
      senseiFirst.push(result)
    } else {
      nonSensei.push(result)
    }
  }
  const partitionedResults = [...senseiFirst, ...nonSensei]
  ...
  const fuseSuggestions = partitionedResults.map(result => {
