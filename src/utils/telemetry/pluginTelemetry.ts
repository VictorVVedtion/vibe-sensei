 ...
 * managedNames=null — the session-level vs_plugin_enabled_for_session
...
/**
 * Emit vs_plugin_enabled_for_session once per enabled plugin at session
 * start. Supplements vs_skill_loaded (which still fires per-skill) — use
...
export function logPluginsEnabledForSession(
  ...
  for (const plugin of plugins) {
    ...
    logEvent('vs_plugin_enabled_for_session', {
      _PROTO_plugin_name:
...
/**
 * Emit vs_plugin_load_failed once per error surfaced by session-start
 * plugin loading. Pairs with vs_plugin_enabled_for_session so dashboards
...
export function logPluginLoadErrors(
  ...
  for (const err of errors) {
    ...
    logEvent('vs_plugin_load_failed', {
      error_category:
