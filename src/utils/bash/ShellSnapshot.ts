...
/**
 * Generates Vibe Sensei specific snapshot content
...
async function getSnapshotScript(
  ...
  // Generate the user content and Vibe Sensei content
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Vibe Sensei defaults only`,
        )
      }
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            const signalNumber = execError?.signal
              ...
              : undefined
            logEvent('vs_shell_snapshot_failed', {
            ...
            if (snapshotSize !== undefined) {
              ...
              logEvent('vs_shell_unknown_error', {})
              resolve(undefined)
            }
          }
        },
      ...
      logError(error)
      logEvent('vs_shell_snapshot_error', {})
      resolve(undefined)
    }
  })
}
