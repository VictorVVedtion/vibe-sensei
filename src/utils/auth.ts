...
export function getAnthropicApiKeyWithSource(
  ...
  // --demo: return a mock key so the app boots without real credentials
  if (process.env.IS_DEMO === '1') {
    return { key: 'sk-demo-00000000000000000000000000000000', source: 'ANTHROPIC_API_KEY' }
  }
...
async function _executeApiKeyHelper(
  ...
  if (isApiKeyHelperFromProjectOrLocalSettings()) {
    ...
    if (!hasTrust && !isNonInteractiveSession) {
      ...
      logEvent('vs_apiKeyHelper_missing_trust11', {})
      return null
    }
  }
...
async function runAwsAuthRefresh(): Promise<boolean> {
  ...
  if (isAwsAuthRefreshFromProjectSettings()) {
    ...
    if (!hasTrust && !getIsNonInteractiveSession()) {
      ...
      logEvent('vs_awsAuthRefresh_missing_trust', {})
      return false
    }
  }
...
async function getAwsCredsFromCredentialExport(): Promise<{
  ...
  if (isAwsCredentialExportFromProjectSettings()) {
    ...
    if (!hasTrust && !getIsNonInteractiveSession()) {
      ...
      logEvent('vs_awsCredentialExport_missing_trust', {})
      return null
    }
  }
...
async function runGcpAuthRefresh(): Promise<boolean> {
  ...
  if (isGcpAuthRefreshFromProjectSettings()) {
    ...
    if (!hasTrust && !getIsNonInteractiveSession()) {
      ...
      logEvent('vs_gcpAuthRefresh_missing_trust', {})
      return false
    }
  }
...
export async function saveApiKey(apiKey: string): Promise<void> {
  ...
  if (process.platform === 'darwin') {
    try {
      ...
      logEvent('vs_api_key_saved_to_keychain', {})
      savedToKeychain = true
    } catch (e) {
      logError(e)
      logEvent('vs_api_key_keychain_error', {
        error: errorMessage(
          e,
        ) as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
      })
      logEvent('vs_api_key_saved_to_config', {})
    }
  } else {
    logEvent('vs_api_key_saved_to_config', {})
  }
...
export function saveOAuthTokensIfNeeded(tokens: OAuthTokens): {
  ...
  if (!shouldUseClaudeAIAuth(tokens.scopes)) {
    logEvent('vs_oauth_tokens_not_claude_ai', {})
    return { success: true }
  }

  // Skip saving inference-only tokens (they come from env vars)
  if (!tokens.refreshToken || !tokens.expiresAt) {
    logEvent('vs_oauth_tokens_inference_only', {})
    return { success: true }
  }
  ...
  try {
    ...
    if (updateStatus.success) {
      logEvent('vs_oauth_tokens_saved', { storageBackend })
    } else {
      logEvent('vs_oauth_tokens_save_failed', { storageBackend })
    }
    ...
    logError(error)
    logEvent('vs_oauth_tokens_save_exception', {
      storageBackend,
      error: errorMessage(
        error,
      ) as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
...
async function handleOAuth401ErrorImpl(
  ...
  if (currentTokens.accessToken !== failedAccessToken) {
    logEvent('vs_oauth_401_recovered_from_keychain', {})
    return true
  }
...
async function checkAndRefreshOAuthTokenIfNeededImpl(
  ...
  let release
  try {
    logEvent('vs_oauth_token_refresh_lock_acquiring', {})
    release = await lockfile.lock(claudeDir)
    logEvent('vs_oauth_token_refresh_lock_acquired', {})
  } catch (err) {
    if ((err as { code?: string }).code === 'ELOCKED') {
      // Another process has the lock, let's retry if we haven't exceeded max retries
      if (retryCount < MAX_RETRIES) {
        logEvent('vs_oauth_token_refresh_lock_retry', {
      ...
      logEvent('vs_oauth_token_refresh_lock_retry_limit_reached', {
        maxRetries: MAX_RETRIES,
      })
      return false
    }
    logError(err)
    logEvent('vs_oauth_token_refresh_lock_error', {
      error: errorMessage(
        err,
      ) as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
    })
  ...
  try {
    ...
    if (
      ...
      logEvent('vs_oauth_token_refresh_race_resolved', {})
      return false
    }

    logEvent('vs_oauth_token_refresh_starting', {})
    ...
    if (currentTokens && !isOAuthTokenExpired(currentTokens.expiresAt)) {
      logEvent('vs_oauth_token_refresh_race_recovered', {})
      return true
    }

    return false
  } finally {
    logEvent('vs_oauth_token_refresh_lock_releasing', {})
    await release()
    logEvent('vs_oauth_token_refresh_lock_released', {})
  }
}
