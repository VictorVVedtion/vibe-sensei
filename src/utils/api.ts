...
export async function toolToAPISchema(
  ...
  // prevent mid-session GrowthBook flips (vs_tool_pear, vs_fgts) or
  ...
  if (!base) {
    const strictToolsEnabled =
      checkStatsigFeatureGate_CACHED_MAY_BE_STALE('vs_tool_pear')
    ...
    if (
      ...
      isFirstPartyAnthropicBaseUrl() &&
      (getFeatureValue_CACHED_MAY_BE_STALE('vs_fgts', false) ||
  ...
  // github.com/VictorVVedtion/vibe-sensei/issues/20031
...
export function splitSysPromptPrefix(
  ...
  if (useGlobalCacheFeature && options?.skipGlobalCacheForSystemPrompt) {
    logEvent('vs_sysprompt_using_tool_based_cache', {
  ...
  if (useGlobalCacheFeature) {
    ...
    if (boundaryIndex !== -1) {
      ...
      logEvent('vs_sysprompt_boundary_found', {
      ...
      logEvent('vs_sysprompt_missing_boundary_marker', {
        promptBlockCount: systemPrompt.length,
      })
    }
  }
  ...
  switch (tool.name) {
    ...
    case BashTool.name: {
      ...
      if (/^echo\s+["']?[^|&;><]*["']?$/i.test(normalizedCommand.trim())) {
        logEvent('vs_bash_tool_simple_echo', {})
      }
