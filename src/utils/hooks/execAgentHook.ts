  ...
  try {
    ...
    try {
      ...
      const systemPrompt = asSystemPrompt([
        `You are verifying a stop condition in Vibe Sensei. Your task is to verify that the agent completed the given plan. The conversation transcript is available at: ${transcriptPath}\nYou can read this file to analyze the conversation history if needed.
...
- ok: false with reason if the condition is not met`,
      ...
      if (!structuredOutputResult) {
        ...
        if (hitMaxTurns) {
          ...
          logEvent('vs_agent_stop_hook_max_turns', {
        ...
        logEvent('vs_agent_stop_hook_error', {
          durationMs: Date.now() - hookStartTime,
          turnCount,
      ...
      logEvent('vs_agent_stop_hook_success', {
        durationMs: Date.now() - hookStartTime,
        turnCount,
        agentName:
          agentName as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
