...
async function handleSessionFileAccess(
  ...
  if (fileType === 'session_memory') {
    logEvent('vs_session_memory_accessed', { ...subagentProps })
  } else if (fileType === 'session_transcript') {
    logEvent('vs_transcript_accessed', { ...subagentProps })
  }
  ...
  if (filePath && isAutoMemFile(filePath)) {
    ...
    switch (input.tool_name) {
      case FILE_READ_TOOL_NAME:
        logEvent('vs_memdir_file_read', { ...subagentProps })
        break
      case FILE_EDIT_TOOL_NAME:
        logEvent('vs_memdir_file_edit', { ...subagentProps })
        break
      case FILE_WRITE_TOOL_NAME:
        logEvent('vs_memdir_file_write', { ...subagentProps })
        break
    }
  }
  ...
  if (feature('TEAMMEM') && filePath && teamMemPaths!.isTeamMemFile(filePath)) {
    ...
    switch (input.tool_name) {
      case FILE_READ_TOOL_NAME:
        logEvent('vs_team_mem_file_read', { ...subagentProps })
        break
      case FILE_EDIT_TOOL_NAME:
        logEvent('vs_team_mem_file_edit', { ...subagentProps })
        teamMemWatcher?.notifyTeamMemoryWrite()
        break
      case FILE_WRITE_TOOL_NAME:
        logEvent('vs_team_mem_file_write', { ...subagentProps })
        teamMemWatcher?.notifyTeamMemoryWrite()
        break
    }
  }
