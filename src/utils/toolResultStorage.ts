 ...
 */
const PERSIST_THRESHOLD_OVERRIDE_FLAG = 'vs_satin_quoll'
...
export function getPersistenceThreshold(
  ...
  // before the GB override so vs_satin_quoll can't force it back on.
...
async function maybePersistLargeToolResult(
  ...
  // Log analytics
  logEvent('vs_tool_result_persisted', {
    toolName: sanitizeToolNameForAnalytics(toolName),
 ...
 * (vs_hawthorn_window) wins when present and a finite positive number;
...
export async function enforceToolResultBudget(
  ...
  for (const [candidate, replacement] of freshReplacements) {
    ...
    logEvent('vs_tool_result_persisted_message_budget', {
  ...
  if (newlyReplaced.length > 0) {
    ...
    logEvent('vs_message_level_tool_result_budget_enforced', {
