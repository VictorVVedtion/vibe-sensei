...
type CronSchedulerOptions = {
   ...
   * vs_kairos_cron gate off stops already-running schedulers (not just
...
export function createCronScheduler(
  ...
  async function load(initial: boolean) {
    ...
    if (missed.length > 0) {
      ...
      logEvent('vs_scheduled_task_missed', {
        count: missed.length,
        taskIds: missed
          .map(t => t.id)
          .join(
  ...
  function check() {
    ...
    function process(t: CronTask, isSession: boolean) {
      ...
      logEvent('vs_scheduled_task_fire', {
      ...
      if (aged) {
        ...
        logEvent('vs_scheduled_task_expired', {
          taskId:
            t.id as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
          ageHours,
        })
