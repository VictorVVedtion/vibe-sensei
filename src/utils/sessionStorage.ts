  ...
  private async persistToRemote(sessionId: UUID, entry: TranscriptMessage) {
    ...
    if (this.internalEventWriter) {
      try {
        ...
        logEvent('vs_session_persistence_failed', {})
...
function applyPreservedSegmentRelinks(
  ...
  if (segIsLive) {
    ...
    if (!reachedHead) {
      ...
      logEvent('vs_relink_walk_broken', {
...
function applySnipRemovals(messages: Map<UUID, TranscriptMessage>): void {
  ...
  logEvent('vs_snip_resume_filtered', {
...
export function buildConversationChain(
  ...
  while (currentMsg) {
    if (seen.has(currentMsg.uuid)) {
      ...
      logEvent('vs_chain_parent_cycle', {})
      break
    }
...
function recoverOrphanedParallelToolResults(
  ...
  logEvent('vs_chain_parallel_tr_recovered', {
 ...
 * point. Emits vs_resume_consistency_delta for BigQuery monitoring of
...
export function checkResumeConsistency(chain: Message[]): void {
  for (let i = chain.length - 1; i >= 0; i--) {
    ...
    const actual = i
    logEvent('vs_resume_consistency_delta', {
      expected,
      actual,
...
async function trackSessionBranchingAnalytics(
  ...
  logEvent('vs_session_forked_branches_fetched', {
 ...
 * - Metrics: `vs_session_renamed` is not fired for AI titles.
 *
...
export async function linkSessionToPR(
  ...
  logEvent('vs_session_linked_to_pr', { prNumber })
}
...
export async function loadTranscriptFile(
  ...
  if (getFeatureValue_CACHED_MAY_BE_STALE('vs_pebble_leaf_prune', false)) {
  ...
  if (hasCycle) {
    logEvent('vs_transcript_parent_cycle', {})
  }
