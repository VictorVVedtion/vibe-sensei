...
function logMemoryDirCounts(
  ...
  void fs.readdir(memoryDir).then(
    ...
    () => {
      ...
      logEvent('vs_memdir_loaded', baseMetadata)
    },
  )
}
...
export function buildSearchingPastContextSection(autoMemDir: string): string[] {
  if (!getFeatureValue_CACHED_MAY_BE_STALE('vs_coral_fern', false)) {
    return []
  }
...
export async function loadMemoryPrompt(): Promise<string | null> {
  ...
  // means the !autoEnabled case falls through to the vs_memdir_disabled
  ...
  if (getFeatureValue_CACHED_MAY_BE_STALE('vs_herring_clock', false)) {
    logEvent('vs_team_memdir_disabled', {})
  }
  return null
}
